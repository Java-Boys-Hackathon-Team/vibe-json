...
import ru.javaboys.vibejson.llm.impls.gulyamov.dto.ActivityDto;
import ru.javaboys.vibejson.llm.impls.gulyamov.dto.ActivityPromptDto;
import ru.javaboys.vibejson.llm.impls.gulyamov.dto.ActivityResultJsonDto;
    ...
    private List<ActivityResultJsonDto> callAllActivityPromptsAsync(List<ActivityPromptDto> prompts) {
        List<CompletableFuture<ActivityResultJsonDto>> futures = prompts.stream()
                .map(ap -> CompletableFuture.supplyAsync(() -> {
                    // 2) Получаем ответ
                    String rawJson = sendPrompt(ap.getPrompt());
